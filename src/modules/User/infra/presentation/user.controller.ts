...
} from '@/modules/User/application/dtos/response-user.dto';
...
export class UserController {
  ...
  @IsAdmin()
  @Post()
  async createUser(@Body() user: CreateUserDTO, @GetUser() adminUser): Promise<UserResponseDTO> {
    return await this.CreateUserService.execute(user, String(adminUser.id));
  }
  ...
  async getUserWithAccount(
    @Param('id') id: string,
    @GetUser() user,
  ): Promise<CreateUserWithAccountResponseDTO> {

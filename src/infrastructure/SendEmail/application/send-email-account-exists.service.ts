...
@Injectable()
export class SendEmailAccountExistsService extends SendEmailHelperIntegration {
  constructor(
  ...
  async execute(email: string, name?: string): Promise<void> {
    const subject = 'Você já tem uma conta na Liga de Algoritmos';
    ...
    try {
      ...
      this.LoggerAdapter.error({
        where: 'SendEmailAccountExistsService',
        message: `Error sending account-exists email to: ${email}. Error: ${error}`,
      });
      throw this.ExceptionsAdapter.internalServerError({
        message: 'Error trying to send account-exists email.',
  ...
  private loadTemplate(name?: string): string {
    try {
      const templatePath = path.join(__dirname, '..', 'templates', 'account-exists-template.html');
      ...
      this.LoggerAdapter.fatal({
        where: 'SendEmailAccountExistsService',
        message: `Error loading account-exists email template. Error: ${error}`,
      });
      throw this.ExceptionsAdapter.internalServerError({
